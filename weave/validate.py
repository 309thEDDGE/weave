...
def validate_pantry(pantry_path, file_system):
    ...
    if not file_system.exists(pantry_path):
        raise ValueError(
            f"Invalid Bucket Path. Bucket does not exist at: {pantry_path}"
        )

    # call check level, with a path, but since we're just starting,
    # we just use the pantry_path as the path
    with warnings.catch_warnings(record=True) as warn:
        _check_level(pantry_path, pantry_path, file_system)
...
def _check_level(pantry_path, current_dir, file_system, in_basket=False):
    ...
    if file_system.exists(manifest_path):
        ...
        if in_basket:
            return True
        return _validate_basket(pantry_path, current_dir, file_system)
    ...
    for file_or_dir in dirs_and_files:
        ...
        if file_type == 'directory':
            ...
            if in_basket:
                return _check_level(pantry_path, file_or_dir,
            ...
            if not _check_level(pantry_path, file_or_dir,
                                file_system, in_basket=in_basket):
                return False
...
def _validate_basket(pantry_path, basket_dir, file_system):
    ...
    for file in files_in_basket:
        ...
        )(pantry_path, file, file_system)

    # default return true if we don't find any problems with this basket
    return True


def _handle_manifest(pantry_path, file, file_system):
    ...
    try:
        ...
        _validate_parent_uuids(pantry_path, data, file_system)
...
def _handle_supplement(_pantry_path, file, file_system):
...
def _handle_metadata(_pantry_path, file, file_system):
...
def _handle_none_of_the_above(pantry_path, file, file_system):
    ...
    if file_system.info(file)['type'] == 'directory':
        if _check_level(pantry_path, file, file_system, in_basket=True):
            warnings.warn(UserWarning(
...
def _validate_parent_uuids(pantry_path, data, file_system):
    ...
    my_index = create_index_from_fs(pantry_path, file_system)
