...
def validate_bucket(bucket_name): 
    ...
    return _check_level(bucket_name, valid_bucket) 


def _check_level(current_dir, valid_bucket, in_basket=False):    
    ...
    if s3fs_client.exists(manifest_path):
        ...
        if in_basket:
            return "Valid Bucket"
        return _validate_basket(current_dir, valid_bucket)
    ...
    for file_or_dir in dirs_and_files:
        ...
        if file_type == 'directory':
            ...
            # and throw an error
            if in_basket:
                if _check_level(file_or_dir, valid_bucket, in_basket=in_basket) == "Valid Bucket":
                    return "Valid Bucket"
                return "Invalid Bucket"
            ...
            # looking at all the sub-directories
                ...
                if _check_level(file_or_dir, valid_bucket, in_basket=in_basket) == "Invalid Bucket":
                    return "Invalid Bucket"
...
def _validate_basket(basket_dir, valid_bucket):   
    ...
    for file in files_in_basket:   
        ...
        if s3fs_client.info(file)['type'] == 'directory':
            if _check_level(file, valid_bucket, in_basket=True) == "Valid Bucket":
                warnings.warn(f"Invalid Basket. "
